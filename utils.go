...
func renderFrame(idx int) {
	...
	features, _ := devcycleClient.AllFeatures(ServiceUser)
	variationName := "Default"
	if feat, exists := features["hello-togglebot"]; exists {
		variationName = feat.VariationName
	}

	variableValue, _ := devcycleClient.VariableValue(ServiceUser, "togglebot-wink", false)
	wink := variableValue.(bool)

	variableValue, _ = devcycleClient.VariableValue(ServiceUser, "togglebot-speed", "off")
	speed := variableValue.(string)
